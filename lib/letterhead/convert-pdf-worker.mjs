...
const orientation =
  process.env.LETTERHEAD_ORIENTATION === 'landscape' ? 'landscape' : 'portrait'
const { widthPx: LETTERHEAD_WIDTH_PX, heightPx: LETTERHEAD_HEIGHT_PX } =
  getLetterheadDimensions(orientation)
...
async function convertPdfFirstPageToPng(pdfBuffer) {
  ...
  try {
    ...
    const outputCanvas = createCanvas(LETTERHEAD_WIDTH_PX, LETTERHEAD_HEIGHT_PX)
    const outputContext = outputCanvas.getContext('2d')

    outputContext.fillStyle = '#ffffff'
    outputContext.fillRect(0, 0, LETTERHEAD_WIDTH_PX, LETTERHEAD_HEIGHT_PX)
    drawContainTop(outputContext, sourceCanvas, LETTERHEAD_WIDTH_PX, LETTERHEAD_HEIGHT_PX)
