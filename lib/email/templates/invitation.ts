interface InvitationEmailParams {
  ...
  email: string
  inviteUrl?: string
  /** @deprecated Legacy temp-password invites for already-provisioned users */
  tempPassword?: string
  loginUrl?: string
  inviterName?: string
  expiresAt?: string
...
export function buildInvitationEmail({
  ...
  const addedByLine = inviterName
    ? `${inviterName} has invited you to join <strong>${orgName}</strong> on Signara.`
    : `You've been invited to join <strong>${orgName}</strong> on Signara.`

  const expiryLine = expiresAt
    ? `<p style="margin:16px 0 0;font-size:13px;color:#718096;line-height:1.5;">This invite expires on ${new Date(
        expiresAt
      ).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}.</p>`
    : ''
                  ...
                  <td style="padding:20px 24px;">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                      ...
                      <tr>
                        ...
                        <td style="padding:6px 0;font-size:14px;color:#0F2C59;font-weight:500;font-family:monospace;">${tempPassword}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
              ...
              <table cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
                <tr>
                  <td style="border-radius:6px;background-color:#D4AF37;">
                    <a href="${loginUrl}" style="display:inline-block;padding:13px 32px;font-size:15px;font-weight:600;color:#0F2C59;text-decoration:none;">Sign in to Signara</a>
                  </td>
                </tr>
              </table>`
      : `<table width="100%" cellpadding="0" cellspacing="0" style="background-color:#F8F9FA;border:1px solid #e0e0e0;border-radius:6px;margin-bottom:28px;">
                <tr>
                  <td style="padding:20px 24px;">
                    <p style="margin:0 0 10px;font-size:13px;font-weight:600;color:#0F2C59;text-transform:uppercase;letter-spacing:0.5px;">Invitation details</p>
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="padding:6px 0;font-size:14px;color:#718096;width:140px;">Email</td>
                        <td style="padding:6px 0;font-size:14px;color:#0F2C59;font-weight:500;">${email}</td>
              ...
              <table cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
                <tr>
                  <td style="border-radius:6px;background-color:#D4AF37;">
                    <a href="${inviteUrl ?? '#'}" style="display:inline-block;padding:13px 32px;font-size:15px;font-weight:600;color:#0F2C59;text-decoration:none;">Accept invitation</a>
                  </td>
                </tr>
              </table>`
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              ...
              <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#4A5568;">
                ...
                ${
                  tempPassword
                    ? ' You can sign in using the temporary credentials below.'
                    : ' Use the button below to set your password and join the organisation.'
                }
              </p>
              ${credentialsBlock}
